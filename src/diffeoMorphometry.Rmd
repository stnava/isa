...
#  dpca = multichannelPCA( wlist, mskpca, k=20, pcaOption=round(sum(mskpca)*0.2), verbose=FALSE  ) 
...
for ( ww in 1:mxk )
  {
  myw = dpca$pcaWarps[[ww]] * ( -0.1 ) / max( abs( dpca$pcaWarps[[ww]]  ) )
...
```{r randspcaresult}
# now make a few "random" face
for ( few in 1:3 ) {
mxk=9
scl = 2
combvec = dpca$pcaWarps[[1]]* rnorm(1, sd=15) * scl * dpca$pca$d[1]/max( dpca$pca$d[1])
for ( i in 2:mxk )
  combvec = combvec + dpca$pcaWarps[[i]] * rnorm(1, sd=15) * scl* dpca$pca$d[i]/max( dpca$pca$d[1])
