...
dpca = multichannelPCA( wlist, mskpca, k=4, pcaOption=50, verbose=FALSE  ) 
#  dpca = multichannelPCA( wlist, mskpca, pcaOption='svd', verbose=FALSE ) # standard
#  dpca = multichannelPCA( wlist, mskpca, pcaOption='randPCA', verbose=FALSE ) # big data standard
...
mypr = abs( cor( pcaReconCoeffs ) )
mypr[ mypr < 0.5  ] = 0
mypr = mypr / 
  Matrix::rowSums( mypr)
imageMetricGrad <- function( pcaParams, fImg, mImg, dpcaIn, metricIn, 
  ...
  for ( k in whichk )
    {
    pcaParamsA = pcaParamsB = pcaParams
    pcaParamsA[k] = pcaParamsA[k] - eps * paramScales[k]
    pcaParamsB[k] = pcaParamsB[k] + eps * paramScales[k]
    for ( i in 1:length( pcaParams ) )
      if ( i == 1 )
        combvec = dpcaIn$pcaWarps[[i]] * pcaParamsA[i] * scl else 
          combvec = combvec + dpcaIn$pcaWarps[[i]] * pcaParamsA[i] * scl
    ...
    for ( i in 1:length( pcaParams ) )
      if ( i == 1 )
        combvec = dpcaIn$pcaWarps[[i]] * pcaParamsB[i] * scl else 
          combvec = combvec + dpcaIn$pcaWarps[[i]] * pcaParamsB[i] * scl
    warpTx = antsrTransformFromDisplacementField( combvec )
    wtxlong = list( ) 
    for ( i in 1:ncomp ) {
      wtxlong[[i]] = warpTx
    ...
    antsrMetricSetMovingImage( metricIn, warped )
    antsrMetricInitialize( metricIn )
    metricVal2 = antsrMetricGetValue( metricIn ) 
    deriv[ k ] = eps * ( metricVal2 - metricVal )
#  print( paste(metricVal2,metricVal))
    }
  return(  as.numeric( parameterRegularization %*% deriv  ) )
}

r16 = antsImageRead( getANTsRData( 'r16' ) )
fixedI = r16 # try the brain - see how improbable a solution we get
fixedI = rilist[[k]]
metric = antsrMetricCreate( fixedI, template, type="MeanSquares")
antsrMetricInitialize( metric )

imageMetric <- function( pcaParams, fImg, mImg, dpcaIn, metricIn, 
                         parameterRegularization ) {
...
sclParams = pcaReconCoeffsSD / max( pcaReconCoeffsSD )
for( poo in 1:1) {
#  drivr = imageMetricGrad( locparams, rilist[[k]], template, dpca, metric,   
#                 parameterRegularization  = mypr, 
#                 paramScales = rep(1,length(sclParams)),
#                 gradStep=1., whichk=1:10 )
#  locparams = locparams - drivr*1e6
#  print( imageMetric( locparams, rilist[[k]], template, dpca, metric   ) )
  }

ctl = list(maxit = 1000, temp = 20, parscale = sclParams )
ctl = list(maxit = 10, parscale = sclParams )
t1=Sys.time()

# pcaRegistration = nlm( f=imageMetric, p=locparams, 
#  metricIn=metric,
#  dpcaIn=dpca, fImg=rilist[[k]], mImg=template,
#  parameterRegularization=mypr, fscale = 0, stepmax = 10 )
# 
# pcaRegistration = optim( locparams, imageMetric, # gr=imageMetricGrad,
#  method = "L-BFGS-B", control = ctl, metricIn=metric,
#  dpcaIn=dpca, fImg=rilist[[k]], mImg=template,
#  parameterRegularization=mypr )
library( neldermead )
library(dfoptim) # derivative free optimization
pcaRegistration <- nmk( locparams, imageMetric, control = list(trace=0,maxfeval=1500),
  metricIn=metric,
  dpcaIn=dpca, fImg=fixedI, mImg=template,
  parameterRegularization=mypr)

t2=Sys.time()
bestParams = pcaRegistration$par
