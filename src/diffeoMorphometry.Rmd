...
# this option can compute > n components
  dpca = multichannelPCA( wlist, mskpca, k=25, pcaOption=50, verbose=FALSE  ) 
...
# dpca = multichannelPCA( wlist, mskpca, k=10, pcaOption='eanat', verbose=FALSE )
...
for ( i in 1:length( wlist ) ) {
  ...
  mdl = lm( wvec ~ dpca$pca$v )
  pcaReconCoeffs[ i,  ] = coefficients(mdl)[-1]
}
...
shapeProbabilities = exp( -1.0 * shapeDistances/ (0.005 * shapeDistancesNorm ) )
shapeProbabilities
...
scl = 1/ncomp
sdscl = 1.2
locparams = pcaReconCoeffsMeans
locparams[ 1 ] = rnorm(1, mean=pcaReconCoeffsMeans[1], sd=pcaReconCoeffsSD[1]*sdscl)
for ( i in 2:mxk ) {
  locparams[ i ] = rnorm(1, mean=pcaReconCoeffsMeans[i], sd=pcaReconCoeffsSD[i]*sdscl)
  }
